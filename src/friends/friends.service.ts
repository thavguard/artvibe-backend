...
export class FriendsService {
    ...
    async findAll(): Promise<FriendsEntity[]> {
        return this.friendsRepository.find({ relations: { friends: true, sentFriends: true, receivedFriends: true } })
    }



    async sendFriendRequest(userId: number, friendId: number): Promise<FriendEntity> {
        const [user, friend] = await this.checkFriendsForUsers(userId, friendId)

        if (user.id === friend.id) {
            throw new BadRequestException('Отсутствие друзей это не повод чтобы добавлять в друзья себя')
        }
    ...
    async acceptFriendRequest(userId: number, friendId: number,): Promise<boolean> {
        ...
        const friendRequest = await this.findFriendRequestByUsers(friend.id, user.id)

        if (!friendRequest || friendRequest?.isAccepted) {
            throw new BadRequestException('Вы уже приняли эту заявку, или ее вовсе не было')
        }

        friendRequest.isAccepted = true

        user.friends.receivedFriends = user.friends.receivedFriends.filter(item => item.id !== friendRequest.id)
        friend.friends.sentFriends = friend.friends.sentFriends.filter(item => item.id !== friendRequest.id)
    ...
    async removeFromFriends(userId: number, friendId: number): Promise<boolean> {
        ...
        user.friends.friends = user.friends.friends.filter(item => item.id !== friend.id)
        friend.friends.friends = friend.friends.friends.filter(item => item.id !== user.id)

        const friendRequest = await this.findFriendRequestByUsers(friend.id, user.id)

        if (!friendRequest) {
            throw new BadRequestException('что-то пошло не так бро')
        }
    ...
    private async createFriendsForUser(userId: number): Promise<FriendsEntity> {
        return this.friendsRepository.save(
            this.friendsRepository.create({ user: { id: userId }, receivedFriends: [], sentFriends: [], friends: [] })
        )
    }

    private async checkFriendsForUsers(userId: number, friendId: number): Promise<[User, User]> {
    ...
    private async createFriend(userId: number, friendId: number): Promise<FriendEntity> {
        return this.friendRepository.save(
            this.friendRepository.create({ user: { id: userId }, friend: { id: friendId } })
        )
    }

    private async findFriendRequestById(requestId: number): Promise<FriendEntity> {
        return this.friendRepository.findOneBy({ id: requestId })
    }

    private async findFriendRequestByUsers(userId: number, friendId: number): Promise<FriendEntity> {
        return this.friendRepository.findOneBy({ user: { id: userId }, friend: { id: friendId } })
    }
}